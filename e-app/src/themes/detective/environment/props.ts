...
/**
 * The furniture of the detective office.
 *
 * Every function returns ops in palette-key form, so the same office can be
 * recoloured by supplying a different ThemePalette. Coordinates are absolute
 * scene pixels; the scene file is the single place that decides layout.
 */
...
function books(x: number, y: number, w: number, seed: number): Op[] {
